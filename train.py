...
parser.add_argument('--num_cls', default=3, type=int, help='number of class in ur cls task')
parser.add_argument('-j', '--workers', default=4, type=int, metavar='N',
                    help='number of data loading workers (default: 4)')
parser.add_argument('--epochs', default=25, type=int, metavar='N',
                    help='number of total epochs to run')
parser.add_argument('--reduce', default=[15, 20], type=list, help='lr decay list')
...
def get_current_lr(epoch, args):
    lr = args.lr
    for i, lr_decay_epoch in enumerate(args.reduce):
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        loss_tmp, acc_tmp = train(train_loader, model, criterion, optimizer, epoch, args)
        loss_all.extend(loss_tmp)
        acc_all.extend(acc_tmp)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        loss_tp.append(loss.item())
        acc_tp.append(acc1[0])
        if i % 500 == 0:
            loss_all.append(sum(loss_tp) / (len(loss_tp)))
            loss_tp = []
            acc_all.append(sum(acc_tp) / (len(acc_tp)))
            acc_tp = []
